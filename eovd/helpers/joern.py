...
def run_joern(filepath: str, verbose: int):
    ...
    joern_path = Path(__file__).parent.parent / "storage" / "external" / "joern-cli" / "joern"
    
    # 设置本地的JAVA_HOME路径
    java_home = Path(__file__).parent.parent / "jdk-11.0.27"
    command = f"JAVA_HOME={java_home} {joern_path} --script {script_file} --params='{params}'"
    if verbose > 2:
        utils.debug(command)
...
def get_node_edges(filepath: str, verbose=0):
    ...
    with open(str(outfile) + ".edges.json", "r") as f:
        ...
        # Convert list of arrays to DataFrame with proper column names
        edges = pd.DataFrame(edges_data, columns=["outnode", "innode", "etype", "dataflow"])
        edges = edges.fillna("")
