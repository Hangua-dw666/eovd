    ...
    def forward(self, x, edge_index, mask_features=False, target_label=None, **kwargs):
        ...
        if kwargs.get('edge_masks'):
            ...
            for ex_label in ex_labels:
                if target_label is None or ex_label[0].item() == target_label[0].item():
                    self.__clear_masks__()
                    self.__set_masks__(x, self_loop_edge_index)
                    edge_mask = self.gnn_explainer_alg(x, edge_index, ex_label, batch=kwargs.get('batch', None)).sigmoid()
