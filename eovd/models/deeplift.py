    ...
    def forward(self,
        ...
        if kwargs.get('edge_masks'):
            ...
            for ex_label in ex_labels:
                ...
                postproc = str(kwargs.get('postproc') or 'raw').strip().lower()
                if postproc in {"relu_pow2", "relu_pow2_smooth1"}:
                    mask = mask.clamp(min=0)
                    smooth_steps = int(kwargs.get('smooth_steps') or (1 if postproc.endswith('smooth1') else 0))
                    smooth_steps = int(max(0, smooth_steps))
                    if smooth_steps > 0:
                        num_nodes = int(mask.shape[0])
                        src = self_loop_edge_index[0]
                        dst = self_loop_edge_index[1]
                        for _ in range(smooth_steps):
                            agg = torch_scatter.scatter_mean(mask[src], dst, dim=0, dim_size=num_nodes)
                            mask = mask + agg
